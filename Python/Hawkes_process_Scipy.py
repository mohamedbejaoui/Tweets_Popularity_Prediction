...
cascade1, cascade2 = cascades.loc[:62], cascades.loc[63:125] 
...
def createStartPoints():
    return([np.random.uniform(0.0,1.0,size=1)[0], np.random.uniform(0.0,1.016,size=1)[0], np.random.uniform(0.0,20.0,size=1)[0], np.random.uniform(0.0,1.0,size=1)[0]])
...
def fitParameters(cascade):
    ...
    res = minimize(fun = neg_log_likelihood,
                   ...
                   bounds = Bounds([0,0,0,0], [1,1.016,9999,999], keep_feasible=False),
                   constraints = NonlinearConstraint(fun = constraint, lb=[-99999999], ub=[0], jac = constraint_jacobian, keep_feasible=False),
                   tol=None, callback=None,
                   options={'maxiter': 500, 'disp': False, 'eps': 1.4901161193847656e-08})
    TT = (time.time() - t0)/60
    return(TT, x0, res.x)

# EXEMPLE
for CC in [cascade1, cascade2]:
    TT, xx0, xx = fitParameters(CC)
    print("###########################################################")
    print("Initial guess x0 = ", xx0)
    print("###########################################################")
    print("Result x_opt = ", xx)
    print("###########################################################")
    print("Computing time = ", TT)
    print("###########################################################\n\n\n\n")
#
