...
def kernel_fct(event, t, K = 0.024, alpha = 2.016, beta = 0.5, mmin = 1, c = 0.001, theta = 0.2, inclusive = True):
	mi = event[0]
	ti = event[1]
...
def lambda_rate(t, history, params, inclusive = False):
	...
	for i in range(len(t)):
		...
		for event_idx in range(len(history_iter)):
			res[i] += kernel_fct(event=history_iter.iloc[event_idx].values,
								  t=np.array([t[i]]),
								  K=K, beta=beta, c=c, theta=theta, inclusive=inclusive)[0]
	return res


def integrate_lambda(upper, history, params, mmin = 1):
...
def neg_log_likelihood(history, params):
...
def closed_gradient(history, params):
...
def constraints(history, params):
...
def fit_parameters(history):
	...
	ub = np.array([1, 1.016, float('inf'), float('inf')])
...
class hawkes_solver(object):
	...
	def constraints(self, x):
		...
		params = dict(K=x[0], beta=x[1], c=x[2], theta=x[3])
		return constraints(self.history, params)

	def jacobian(self, x):
		""" callback for calculating the Jacobian """
		params = dict(K=x[0], beta=x[1], c=x[2], theta=x[3])
		return jacobian(self.history, params)
